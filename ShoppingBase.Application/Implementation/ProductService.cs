...
namespace ShoppingBase.Application.Implementation
{
    ...
    public PagedResult<ProductViewModel> GetByCategoryIdPaging(int? categoryId, string keyword, int page, int pageSize)
    {
        var product = _productRepository.FindAll(x => x.Status == Status.Active);
        if (!string.IsNullOrEmpty(keyword))
            product = product.Where(x => x.Name.Contains(keyword));
            var Category = _productCategoryRepository.FindAll();

            var pId = Category.Where(x => x.Id == categoryId || x.ParentId == categoryId);

            //  query = query.Where(x => x.CategoryId == categoryId.Value);

            //var result = from p in product
            //             where p.CategoryId.Contains(categoryId.ToString());
            //{
            //    var result = from p in product
